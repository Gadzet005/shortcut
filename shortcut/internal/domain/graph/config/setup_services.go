...
func SetupServices(namespaceConfigs map[string]NamespaceConfig, warnUser func (s string)) (map[graph.ID]graph.Graph, error) {
	...
	for namespace, namespaceConfig := range(namespaceConfigs) {
		...
		if err != nil {
			return nil, errorsutils.WrapFail(err, "read service configs in namespace %s", namespace)
		}

		graphConfig, err := readGraphConfigs(namespaceConfig.Graphs)
		if err != nil {
			return nil, errorsutils.WrapFail(err, "read graph configs in namespace %s", namespace)
		}
		...
		for graphName, gc := range(graphConfig) {
			...
			if numRetNodes > 1 {
				return nil, errorsutils.WrapFail(err, "found more than one ret node in graph %s, namespace %s", retGraph.ID, namespace)
			}
...
func readServiceConfigs(configsPath []string) (map[string]graph.Node, *sets.Set[graph.Dependency], error) {
	...
	for _, configPath := range(configsPath) {
		...
		for endpointName, endpoint := range(config.Endpoints) {
			...
			if err != nil {
				return nil, nil, errorsutils.WrapFail(err, "parse url: %s", config.Host)
			}
